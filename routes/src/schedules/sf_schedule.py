...
def print_sf_line(f3x_data, md5_directory, line_number, sf_list, page_cnt, start_page,
                    total_no_of_pages, cord_name=None):
    try:
        if sf_list:
            ...
            for page_num in range(page_cnt):
                ...
                build_sf_per_page_schedule_dict(last_page, last_page_cnt,
                ...
                sf_outfile = md5_directory + 'SF/' + cord_name + '/page_' + str(page_num) + '.pdf'
                ...
                current_page_num = build_memo_page(memo_array, 
                                                md5_directory, cord_name,
                                                current_page_num, page_num, 
                                                total_no_of_pages, sf_outfile, name='SF')
...
def build_sf_per_page_schedule_dict(last_page, transactions_in_page, page_start_index, schedule_page_dict,
    ...
    for index in range(transactions_in_page):
        ...
        for key in schedules[page_start_index]:
            build_payee_sf_name_date_dict(index + 1, key, schedule_dict, schedule_page_dict)
...
def build_payee_sf_name_date_dict(index, key, schedule_dict, schedule_page_dict):
    try:
        ...
        if schedule_dict.get('designatingCommitteeName'):
            schedule_page_dict['designatingCommitteeName'] = schedule_dict['designatingCommitteeName']

        if schedule_dict.get('subordinateCommitteeName'):
            schedule_page_dict['subordinateCommitteeName'] = schedule_dict['subordinateCommitteeName']
            schedule_page_dict['subordinateCommitteeStreet1'] = schedule_dict['subordinateCommitteeStreet1']
            schedule_page_dict['subordinateCommitteeStreet2'] = schedule_dict['subordinateCommitteeStreet2']
            schedule_page_dict['subordinateCommitteeCity'] = schedule_dict['subordinateCommitteeCity']
            schedule_page_dict['subordinateCommitteeState'] = schedule_dict['subordinateCommitteeState']
            schedule_page_dict['subordinateCommitteeZipCode'] = schedule_dict['subordinateCommitteeZipCode']
        ...
        if schedule_dict.get('payeeLastName'):
            ...
            schedule_page_dict["payeeName_" + str(index)] = schedule_dict['payeeOrganizationName']
        elif schedule_dict.get('designatingCommitteeName'):
            schedule_page_dict["payeeName_" + str(index)] = schedule_dict['designatingCommitteeName']
        ...
        if key == 'expenditureDate' and schedule_dict['expenditureDate'] not in ["none", "null", " ", ""]:
            ...
            if key == 'expenditureAmount' or key == 'aggregateGeneralElectionExpended':
                schedule_page_dict[key + '_' + str(index)] = '{0:.2f}'.format(schedule_dict[key]) if schedule_dict[key] else 0.0
            else:
                schedule_page_dict[key + '_' + str(index)] = schedule_dict[key]
    except Exception as e:
        print('Error at key: ' + key + ' in Schedule F transaction: ' + str(schedule_dict))
        raise e
