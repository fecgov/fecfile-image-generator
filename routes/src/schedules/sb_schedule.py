...
def print_sb_line(f3x_data, md5_directory, line_number, sb_list, page_cnt, start_page,
                    total_no_of_pages):
    ...
    if sb_list:
        ...
        for page_num in range(page_cnt):
            ...
            build_sb_per_page_schedule_dict(last_page, last_page_cnt,
            ...
            sb_outfile = md5_directory + 'SB/' + line_number + '/page_' + str(page_num) + '.pdf'
            ...
            current_page_num = build_memo_page(memo_array,
                                            ...
                                            current_page_num, page_num,
                                            total_no_of_pages, sb_outfile, name='SB')
...
def build_sb_per_page_schedule_dict(last_page, transactions_in_page, page_start_index, schedule_page_dict,
    ...
    for index in range(transactions_in_page):
        ...
        for key in schedules[page_start_index]:
            build_payee_name_date_dict(index + 1, key, schedule_dict, schedule_page_dict)
...
def build_payee_name_date_dict(index, key, schedule_dict, schedule_page_dict):
    try:
        ...
        if schedule_dict.get('payeeLastName'):
            ...
            schedule_page_dict["payeeName_" + str(index)] = ",".join(map(str, payee_full_name))
        ...
        if schedule_dict.get('beneficiaryCandidateLastName'):
            ...
            beneficiaryCandidate_full_name.append(schedule_dict.get('beneficiaryCandidateLastName'))
            beneficiaryCandidate_full_name.append(schedule_dict.get('beneficiaryCandidateFirstName'))
            beneficiaryCandidate_full_name.append(schedule_dict.get('beneficiaryCandidateMiddleName'))
            beneficiaryCandidate_full_name.append(schedule_dict.get('beneficiaryCandidatePrefix'))
            beneficiaryCandidate_full_name.append(schedule_dict.get('beneficiaryCandidateSuffix'))

            # removing empty string from beneficiaryCandidate_full_name if any
            beneficiaryCandidate_full_name = list(filter(None, beneficiaryCandidate_full_name))
            schedule_page_dict['beneficiaryName_' + str(index)] = ",".join(map(str, beneficiaryCandidate_full_name))
            

        if key == 'electionCode':
            if schedule_dict[key] and schedule_dict[key][0] in ['P','G']:
                schedule_page_dict['electionType_' + str(index)] = schedule_dict[key][0:1]
            else:
        ...
        if key == 'expenditureDate':
            ...
            if key == 'expenditureAmount' or key == 'expenditureAggregate':
                schedule_page_dict[key + '_' + str(index)] = '{0:.2f}'.format(schedule_dict[key])
            else:
                schedule_page_dict[key + '_' + str(index)] = schedule_dict[key]
    except Exception as e:
        print('Error at key: ' + key + ' in Schedule B transaction: ' + str(schedule_dict))
        raise e
