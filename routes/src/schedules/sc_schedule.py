...
def print_sc_line(f3x_data, md5_directory, sc_schedules, sc_start_page, total_no_of_pages):
    try:
        sc_schedule_total = 0
        ...
        for sc in sc_schedules:
            ...
            for item in ['memoCode', 'memoDescription', 'lenderStreet1', 'lenderStreet2', 'lenderCity', 'lenderState', 'lenderZipCode', 'electionOtherDescription', 'isLoanSecured']:
                sc_schedule_page_dict[item] = sc.get(item)

            for item in ['loanAmountOriginal','loanPaymentToDate', 'loanBalance', 'loanInterestRate']:
            ...
            if not sc.get('lenderOrganizationName'):
                lenderName = ""
                for item in ['lenderLastName', 'lenderFirstName', 'lenderMiddleName', 'lenderPrefix', 'lenderSuffix']:
                    if sc.get(item):
            ...
            if sc.get('child'):
                ...
                if sc2:
                    ...
                    for i in range(len(sc2_list)):
                        ...
                        for j in range(len(sc2_list[i])):
                            sc2_name = ""
                            for k in ['prefix', 'lastName', 'firstName', 'middleName', 'suffix']:
                                if sc2_list[i][j].get(k) != "":
                                    sc2_name += sc2_list[i][j].get(k) + ","

                            sc_schedule_single_page_dict['name_{}'.format(j+1)] = sc2_name[0:-1]
                            sc_schedule_single_page_dict['street1_{}'.format(j+1)] = sc2_list[i][j].get('street1')
                            sc_schedule_single_page_dict['street2_{}'.format(j+1)] = sc2_list[i][j].get('street2')
                            sc_schedule_single_page_dict['city_{}'.format(j+1)] = sc2_list[i][j].get('city')
                            sc_schedule_single_page_dict['state_{}'.format(j+1)] = sc2_list[i][j].get('state')
                            sc_schedule_single_page_dict['zipCode_{}'.format(j+1)] = sc2_list[i][j].get('zipCode')
                            sc_schedule_single_page_dict['employer_{}'.format(j+1)] = sc2_list[i][j].get('employer')
                            sc_schedule_single_page_dict['occupation_{}'.format(j+1)] = sc2_list[i][j].get('occupation')
                            sc_schedule_single_page_dict['guaranteedAmount_{}'.format(j+1)] = '{0:.2f}'.format(float(sc2_list[i][j].get('guaranteedAmount')))
                        
                        sc_schedule_single_page_dict['pageNo'] = sc_start_page + page_num
                        page_num += 1
                        if sc_schedules[len(sc_schedules)-1].get('transactionId') == sc_schedule_single_page_dict.get('TRANSACTION_ID') and i == len(sc2_list)-1:
                        ...
                        if sc_schedule_page_dict.get('memoDescription'):
                            ...
                            memo_dict['PAGESTR'] = "PAGE " + str(sc_start_page + page_num) + " / " + str(total_no_of_pages)
                ...
                if path.isfile(md5_directory + 'SC/all_pages.pdf'):
                    ...
                    os.rename(md5_directory + 'SC' + '/page_' + str(sc_start_page) + '.pdf', md5_directory + 'SC/all_pages.pdf')
        return sc1_list, sc_start_page + page_num - 1, totalOutstandingLoans
    except:
