...
def print_sh2_line(f3x_data, md5_directory, tran_type_ident, sh2_list, page_cnt, start_page,
                    total_no_of_pages):
    try:
        if sh2_list:
            ...
            for page_num in range(page_cnt):
                ...
                build_sh2_per_page_schedule_dict(last_page, last_page_cnt,
...
def build_sh2_per_page_schedule_dict(last_page, transactions_in_page, page_start_index, schedule_page_dict,
                                     sh2_schedules):
    if not last_page:
    ...
    try:
        for key in schedule_dict:
            if key in ['fundraising','directCandidateSupport']:
                schedule_dict[key] = str(schedule_dict[key])
        ...
        print('Error at key: ' + key + ' in Schedule SH2 transaction: ' + str(schedule_dict))
        raise e
