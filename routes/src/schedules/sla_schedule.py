...
def print_sla_line(f3x_data, md5_directory, line_number, sla_list, page_cnt, start_page,
                    total_no_of_pages):
    try:
        if sla_list:
            ...
            for page_num in range(page_cnt):
                ...
                build_la_per_page_schedule_dict(last_page, last_page_cnt,
                ...
                la_outfile = md5_directory + 'SL-A/' + line_number + '/page_' + str(page_num) + '.pdf'
                ...
                if len(memo_array) >= 1:
                    ...
                    memo_outfile = md5_directory + 'SL-A/' + line_number + '/page_memo_' + str(page_num) + '.pdf'
...
def build_la_per_page_schedule_dict(last_page, transactions_in_page, page_start_index, schedule_page_dict,
                                    sla_schedules, memo_array):
    page_subtotal = 0
    ...
    try:
        ...
        for index in range(transactions_in_page):
            ...
            build_contributor_la_name_date_dict(index + 1, page_start_index, schedule_dict, schedule_page_dict)
...
def build_contributor_la_name_date_dict(index, key, schedule_dict, schedule_page_dict):
    ...
    try:
        ...
        print('Error at key: ' + key + ' in Schedule SL-A tranlaction: ' + str(schedule_dict))
        raise e
