...
def print_sl_levin(f3x_data, md5_directory, levin_name, sl_list, page_cnt, start_page,
                    last_page_cnt, total_no_of_pages):
    ...
    try:
        if sl_list:
            ...
            for sl_page_no in range(page_cnt):
                ...
                build_sl_levin_per_page_schedule_dict(last_page, last_page_cnt,
...
def build_sl_levin_per_page_schedule_dict(last_page, transactions_in_page, page_start_index, sl_levin_schedule_page_dict,
                                    sl_levin_schedules):
    page_subtotal = 0
    try:
        if not last_page:
        ...
        for index in range(transactions_in_page):
            ...
            build_contributor_sl_levin_name_date_dict(index + 1, page_start_index, sl_levin_schedule_dict, sl_levin_schedule_page_dict)
    except Exception as e:
...
def build_contributor_sl_levin_name_date_dict(index, key, sl_schedule_dict, sl_schedule_page_dict):
    ...
    try:
        list_SL_convert_2_decimals = [
            'itemizedReceiptsFromPersons', 'unitemizedReceiptsFromPersons',
            'totalReceiptsFromPersons', 'otherReceipts', 'totalReceipts',
            'voterRegistrationDisbursements', 'voterIdDisbursements', 'gotvDisbursements',
            'genericCampaignDisbursements', 'totalSubDisbursements', 'otherDisbursements', 
            'totalDisbursements', 'beginningCashOnHand', 'receipts', 'subtotal',
            ...
            'otherDisbursementsYTD', 'totalDisbursementsYTD', 'beginningCashOnHandYTD', 
            'receiptsYTD', 'subtotalYTD', 'disbursementsYTD', 'endingCashOnHandYTD']
