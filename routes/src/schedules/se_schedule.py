...
def print_se_line(f3x_data, md5_directory, line_number, se_list, page_cnt, start_page,
                    total_no_of_pages):
    try:
        if se_list:
            ...
            for page_num in range(page_cnt):
                ...
                build_se_per_page_schedule_dict(last_page, last_page_cnt,
                ...
                se_outfile = md5_directory + 'SE/' + line_number + '/page_' + str(page_num) + '.pdf'
                ...
                if len(memo_array) >= 1:
                    ...
                    memo_outfile = md5_directory + 'SE/' + line_number + '/page_memo_' + str(page_num) + '.pdf'
...
def build_se_per_page_schedule_dict(last_page, transactions_in_page, page_start_index, schedule_page_dict,
...
def build_se_name_date_dict(index, key, schedule_dict, schedule_page_dict):
    try:
        ...
        elif se_schedule_dictget('payeeOrganizationName'):
            schedule_page_dict["payeeName_" + str(index)] = schedule_dict['payeeOrganizationName']

        if se_schedule_dictget('candidateLastName'):
            name = ''
            for item in name_list:
                item = 'candidate' + item
                if schedule_dict.get(item):
        ...
        if se_schedule_dictget('completingLastName'):
            name = ''
            for item in name_list:
        ...
        print('Error at key: ' + key + ' in Schedule E transaction: ' + str(schedule_dict))
        raise e
