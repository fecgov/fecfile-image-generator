...
def print_sa_line(
    ...
    if sa_list:
        ...
        for page_num in range(page_cnt):
            ...
            sa_outfile = (
                md5_directory + "SA/" + line_number + "/page_" + str(page_num) + ".pdf"
            )
...
def build_sa_per_page_schedule_dict(
    ...
    for index in range(transactions_in_page):
        ...
        build_contributor_name_date_dict(
            index + 1, start_index, schedule_dict, schedule_page_dict
        )
...
def build_contributor_name_date_dict(index, key, schedule_dict, schedule_page_dict):
    try:
        ...
        print(
            "Error at key: " + key + " in Schedule A transaction: " + str(schedule_dict)
        )
        raise e
