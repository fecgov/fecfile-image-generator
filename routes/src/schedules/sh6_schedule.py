...
def print_sh6_line(
    ...
    if sh6_list:
        ...
        for page_num in range(page_cnt):
            ...
            build_sh6_line_per_page_schedule_dict(
                last_page,
                last_page_cnt,
                page_start_index,
                schedule_page_dict,
            ...
            schedule_page_dict["fedLevinSubTotalShare"] = (
            ...
            if page_cnt == page_num + 1:
                schedule_page_dict["totalFederalShare"] = "{0:.2f}".format(
                    total_federal_share
                )
                schedule_page_dict["totallevinShare"] = "{0:.2f}".format(
                    total_levin_share
                )
                schedule_page_dict["fedLevinTotalShare"] = (
                    total_federal_share + total_levin_share
                )
            schedule_page_dict["committeeName"] = f3x_data["committeeName"]
            sh6_outfile = (
                md5_directory + "SH6/" + line_number + "/page_" + str(page_num) + ".pdf"
            )
...
def build_sh6_line_per_page_schedule_dict(
    ...
    schedule_page_dict["fedLevinSubTotalShare"] = float(
        schedule_page_dict["subTotalFederalShare"]
    ) + float(schedule_page_dict["subTotalLevinShare"])
