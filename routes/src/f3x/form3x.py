...
from routes.src.f3x.helper import calculate_page_count
...
from routes.src.f3x.line_numbers import process_sa_line_numbers, process_sf_line_numbers, process_sh_line_numbers, process_line_numbers
...
def print_pdftk(stamp_print):
	...
	try: 
		if 'json_file' in request.files:
			...
			page_count = True if request.form.get('page_count') and request.form.get('page_count').lower() in ['true', '1'] else False
			...
			# process all schedules and build the PDF's if not page_count is False
			process_output, total_no_of_pages = process_schedules_pages(f3x_data, md5_directory, total_no_of_pages, page_count)
			...
			if f3x_summary and not page_count:
				...
				for key in schedule_key_list:
					if key == "has_sa_schedules":
						if schedule_dict[key][0]:
							pypdftk.concat([md5_directory + 'F3X_Summary.pdf', md5_directory + schedule_dict[key][1] + '/all_pages.pdf'], md5_directory + 'all_pages.pdf')
						...
						pypdftk.concat([md5_directory + 'all_pages.pdf', md5_directory + schedule_dict[key][1] + '/all_pages.pdf'], md5_directory + 'temp_all_pages.pdf')
				...
				for key in schedule_key_list:
					if key == "has_sa_schedules" and schedule_dict[key][0]:
						shutil.move(md5_directory + schedule_dict[key][1] + 'SA/all_pages.pdf', md5_directory + 'all_pages.pdf')
						shutil.rmtree(md5_directory + schedule_dict[key][1])
						...
						if path.exists(md5_directory + 'all_pages.pdf'):
							...
							shutil.move(md5_directory + schedule_dict[key][1] + '/all_pages.pdf', md5_directory + 'all_pages.pdf')
						shutil.rmtree(md5_directory + schedule_dict[key][1])
			...
			if flask.request.method == "POST":
				...
				status_code = status.HTTP_201_CREATED if not page_count else status.HTTP_200_OK
				return flask.jsonify(**envelope), status_code
...
def process_schedules_pages(f3x_data, md5_directory, total_no_of_pages, page_count=False):
	...
	# check if schedules exist in data file
		if 'schedules' in f3x_data:
			...
			if schedules.get('SC'):
				...
				total_sc_pages = len(sc_schedules) + sc1_schedules_cnt + sc1_memo_page_cnt + sc_memo_page_cnt + additional_sc_pg_cnt
				...
				for schedule in sd_schedules:
					...
					if schedule.get('child'):
						...
						for child_schedule in child_schedules:
							if child_schedule.get('transactionTypeIdentifier') in ['OPEXP_DEBT', 'FEA_100PCT_DEBT_PAY', 'OTH_DISB_DEBT']:
			...
			if schedules.get('SA'):
				...
				sa_line_numbers_dict['11A1'] = {'data': [], 'page_cnt': 0, 'memo_page_cnt': 0}
				sa_line_numbers_dict['11B'] = {'data': [], 'page_cnt': 0, 'memo_page_cnt': 0}
				sa_line_numbers_dict['11C'] = {'data': [], 'page_cnt': 0, 'memo_page_cnt': 0}
				sa_line_numbers_dict['12'] = {'data': [], 'page_cnt': 0, 'memo_page_cnt': 0}
				sa_line_numbers_dict['13'] = {'data': [], 'page_cnt': 0, 'memo_page_cnt': 0}
				sa_line_numbers_dict['14'] = {'data': [], 'page_cnt': 0, 'memo_page_cnt': 0}
				sa_line_numbers_dict['15'] = {'data': [], 'page_cnt': 0, 'memo_page_cnt': 0}
				sa_line_numbers_dict['16'] = {'data': [], 'page_cnt': 0, 'memo_page_cnt': 0}
				sa_line_numbers_dict['17'] = {'data': [], 'page_cnt': 0, 'memo_page_cnt': 0}
				...
				# calculate number of pages for each SA line numbers 
				# and add to total_no_of_pages
				for key, value in sa_line_numbers_dict.items():
					value['page_cnt'], value['memo_page_cnt'] = calculate_page_count(schedules=value['data'], num=3)
			...
			if schedules.get('SB'):
				...
				sb_line_numbers_dict['21B'] = {'data': [], 'page_cnt': 0, 'memo_page_cnt': 0}
				sb_line_numbers_dict['22'] = {'data': [], 'page_cnt': 0, 'memo_page_cnt': 0}
				sb_line_numbers_dict['23'] = {'data': [], 'page_cnt': 0, 'memo_page_cnt': 0}
				sb_line_numbers_dict['26'] = {'data': [], 'page_cnt': 0, 'memo_page_cnt': 0}
				sb_line_numbers_dict['27'] = {'data': [], 'page_cnt': 0, 'memo_page_cnt': 0}
				sb_line_numbers_dict['28A'] = {'data': [], 'page_cnt': 0, 'memo_page_cnt': 0}
				sb_line_numbers_dict['28B'] = {'data': [], 'page_cnt': 0, 'memo_page_cnt': 0}
				sb_line_numbers_dict['28C'] = {'data': [], 'page_cnt': 0, 'memo_page_cnt': 0}
				sb_line_numbers_dict['29'] = {'data': [], 'page_cnt': 0, 'memo_page_cnt': 0}
				sb_line_numbers_dict['30B'] = {'data': [], 'page_cnt': 0, 'memo_page_cnt': 0}

				# process for each Schedule B
				for schedule in sb_schedules:
					if schedule.get('child'):
						sb_child_schedules = schedule['child']

						for sb_child_schedule in sb_child_schedules:
							if sb_child_schedule['lineNumber'] in sb_line_numbers:
								sb_schedules.append(sb_schedule_child)
			...
			if schedules.get('SE'):
				...
				se_line_numbers_dict['24'] = {'data': [], 'page_cnt': 0, 'memo_page_cnt': 0}
			...
			if schedules.get('SF'):
				...
				for schedule in sf_schedules:	
					process_sf_line_numbers(
								sf_crd, sf_non_crd, sf_empty_ord, sf_empty_non_ord, sf_empty_none, 
								sf_empty_sub, sf_crd_memo, sf_non_crd_memo, sf_empty_ord_memo, 
								sf_empty_non_ord_memo, sf_empty_none_memo, sf_empty_sub_memo, schedule)			
				...
				for lis in list_dirs:
					...
					for val in values:
						sf_crd_page_cnt, sf_crd_memo_page_cnt = calculate_page_count(schedules=val, num=3)
			...
			if schedules.get('SH'):
				...
				sh_line_numbers_dict['H1'] = {'data': [], 'page_cnt': 0, 'start_page': 0, 'methodName': print_sh1_line}
				sh_line_numbers_dict['H2'] = {'data': [], 'page_cnt': 0, 'start_page': 0, 'methodName': print_sh2_line}
				sh_line_numbers_dict['18A'] = {'data': [], 'page_cnt': 0, 'start_page': 0, 'methodName': print_sh3_line}
				sh_line_numbers_dict['18B'] = {'data': [], 'page_cnt': 0, 'memo_page_cnt': 0, 'start_page': 0, 'methodName': print_sh5_line}
				sh_line_numbers_dict['21A'] = {'data': [], 'page_cnt': 0, 'memo_page_cnt': 0, 'start_page': 0, 'methodName': print_sh4_line}
				sh_line_numbers_dict['30A'] = {'data': [], 'page_cnt': 0, 'memo_page_cnt': 0, 'start_page': 0, 'methodName': print_sh6_line}
				...
				for key, value in sh_line_numbers_dict.items():
					if value['data']:
						if key == 'H1':
							...
							value['page_cnt'], _ = calculate_page_count(schedules=value['data'], num=6)
							...
							value['page_cnt'], _ = calculate_page_count(schedules=value['data'], num=1)
							total_no_of_pages += value['page_cnt']
							...
							value['page_cnt'], value['memo_page_cnt'] = calculate_page_count(schedules=value['data'], num=2)
							total_no_of_pages += value['page_cnt'] + value['memo_page_cnt']
			...
			if schedules.get('SL-A'):
				...
				sla_line_numbers_dict['1A'] = {'data': [], 'page_cnt': 0, 'memo_page_cnt': 0}
				sla_line_numbers_dict['2'] = {'data': [], 'page_cnt': 0, 'memo_page_cnt': 0}
				...
				for key, value in sla_line_numbers_dict.items():
					value['page_cnt'], value['memo_page_cnt'] = calculate_page_count(schedules=value['data'], num=4)
					total_no_of_pages += value['page_cnt'] + value['memo_page_cnt']
			...
			if schedules.get('SL-B'):
				...
				slb_line_numbers_dict['4A'] = {'data': [], 'page_cnt': 0, 'memo_page_cnt': 0}
				slb_line_numbers_dict['4B'] = {'data': [], 'page_cnt': 0, 'memo_page_cnt': 0}
				slb_line_numbers_dict['4C'] = {'data': [], 'page_cnt': 0, 'memo_page_cnt': 0}
				slb_line_numbers_dict['4D'] = {'data': [], 'page_cnt': 0, 'memo_page_cnt': 0}
				slb_line_numbers_dict['5'] = {'data': [], 'page_cnt': 0, 'memo_page_cnt': 0}
				...
				for key, value in slb_line_numbers_dict.items():
					value['page_cnt'], value['memo_page_cnt'] = calculate_page_count(schedules=value['data'], num=5)
					total_no_of_pages += value['page_cnt'] + value['memo_page_cnt']
		...
		if not page_count:
			...
			if sa_schedules:
				...
				for key, value in sa_line_numbers_dict.items():
					if value['data']:
						current_page_num = print_sa_line(f3x_data, md5_directory, key, value['data'], 
                                       value['page_cnt'], current_page_num, total_no_of_pages)
			...
			if sb_schedules:
				...
				for key, value in sb_line_numbers_dict.items():
					if value['data']:
						current_page_num = print_sb_line(f3x_data, md5_directory, key, value['data'], 
                                       value['page_cnt'], current_page_num, total_no_of_pages)


			if sc_schedules:
				sc1_list, sc1_start_page, totalOutstandingLoans = print_sc_line(f3x_data, md5_directory, sc_schedules, sc_start_page, total_no_of_pages)
			else:
				sc1_list = []

			if sc1_list:
				for sc1 in sc1_list:
					print_sc1_line(f3x_data, md5_directory, sc1, sc1_start_page, total_no_of_pages)
					sc1_start_page += 1
		

			if sd_schedules:
					sd_total_balance = print_sd_line(f3x_data, md5_directory, sd_dict, sd_start_page, total_no_of_pages, total_sd_pages, totalOutstandingLoans)
			...
			if se_schedules:
				...
				for key, value in se_line_numbers_dict.items():
					if value['data']:
						current_page_num = print_se_line(f3x_data, md5_directory, key, value['data'], 
                                                    value['page_cnt'], se_24_start_page, total_no_of_pages)
			...
			if sf_schedules:
				...
				for lis in list_dirs:
					...
					for rec in values:
						...
						sf_crd_page_cnt, sf_crd_memo_page_cnt = calculate_page_count(schedules=rec, num=3)
						if count == 0:
							count += 1
							sf_crd_start_page = sf_start_page
							print_sf_line(f3x_data, md5_directory, '25', rec, sf_crd_page_cnt, sf_crd_start_page,
										total_no_of_pages, cord_name)				
						elif count == 1:
							count += 1
							sf_non_crd_start_page = sf_crd_start_page + sf_crd_page_cnt + sf_crd_memo_page_cnt
							print_sf_line(f3x_data, md5_directory, '25', rec, sf_crd_page_cnt, sf_non_crd_start_page,
											total_no_of_pages, cord_name)						
						else:
							count += 1
							sf_non_crd_start_page = sf_non_crd_start_page + sf_crd_page_cnt + sf_crd_memo_page_cnt
							print_sf_line(f3x_data, md5_directory, '25', rec, sf_crd_page_cnt, sf_non_crd_start_page,
											total_no_of_pages, cord_name)
			...
			if sh_schedules:
				for key, value in sh_line_numbers_dict.items():
					if value['data']:
						if key in ['H1', 'H2']:
							tran_type_ident = value['data'][0]['transactionTypeIdentifier']
							if tran_type_ident:
								value['methodName'](f3x_data, md5_directory, tran_type_ident, value['data'], value['page_cnt'], value['start_page'],
									total_no_of_pages)

						else:
							value['methodName'](f3x_data, md5_directory, key, value['data'], value['page_cnt'], value['start_page'],
											total_no_of_pages)
			...
			if sl_summary and levin_name_data:
				...
				print_sl_levin(f3x_data, md5_directory, name, account_data, sl_levin_page_cnt, sl_start_page,
			...
			if sla_schedules:
				...
				for key, value in sla_line_numbers_dict.items():
					if value['data']:
						print_sla_line(f3x_data, md5_directory, key, value['data'], 
                                       value['page_cnt'], current_page_num, total_no_of_pages)
			...
			if slb_schedules:
				...
				for key, value in slb_line_numbers_dict.items():
					if value['data']:
						print_slb_line(f3x_data, md5_directory, key, value['data'], 
                                       value['page_cnt'], current_page_num, total_no_of_pages)
						current_page_num += value['page_cnt'] + value['memo_page_cnt']
