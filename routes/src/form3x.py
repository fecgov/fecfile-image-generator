...
from routes.src import utils, common
...
def print_pdftk(stamp_print):
    ...
    try:
        if "json_file" in request.files:
            ...
            has_sa_schedules = (
                has_sb_schedules
            ) = has_la_schedules = has_slb_schedules = has_sl_summary = False
            ...
            has_sh6_schedules = (
                ...
                has_sh1_schedules
            ) = has_sh2_schedules = has_sf_schedules = has_se_schedules = False
            ...
            process_output, total_no_of_pages = process_schedules(
                f3x_data, md5_directory, total_no_of_pages
            )
            ...
            if len(f3x_summary) > 0:
                ...
                if has_sh6_schedules:
                    pypdftk.concat(
                        [
                            md5_directory + "all_pages.pdf",
                            md5_directory + "SH6/all_pages.pdf",
                        ],
                        md5_directory + "temp_all_pages.pdf",
                    )
                    shutil.move(
...
def process_schedules(f3x_data, md5_directory, total_no_of_pages):
    ...
    has_sc_schedules = (
        ...
        has_la_schedules
    ) = has_slb_schedules = has_sf_schedules = has_se_schedules = False

    has_sh6_schedules = (
        has_sh4_schedules
    ) = (
        has_sh5_schedules
    ) = has_sh3_schedules = has_sh1_schedules = has_sh2_schedules = False
    sa_schedules_cnt = (
        sb_schedules_cnt
    ) = (
        sh_schedules_cnt
        ...
        sh3_schedules_cnt
    ) = sh1_schedules_cnt = sh2_schedules_cnt = sf_schedules_cnt = se_schedules_cnt = 0
    ...
    if "schedules" in f3x_data:
                ...
                for sc_count in range(sc_schedules_cnt):
                    if "child" in sc_schedules[sc_count]:
                        ...
                        for sc_child_count in range(sc_child_schedules_count):
                            if (
                                sc_schedules[sc_count]
                                .get("child")[sc_child_count]
                                .get("lineNumber")
                                in sc_sa_line_numbers
                            ):
                                if "SA" not in schedules:
                                    schedules["SA"] = []
                                schedules["SA"].append(
                                    sc_schedules[sc_count]["child"][sc_child_count]
                                )
                            elif (
                                sc_schedules[sc_count]["child"][sc_child_count][
                                    "lineNumber"
                                ]
                                in sc_sb_line_numbers
                            ):
                            ...
                            elif sc_schedules[sc_count]["child"][sc_child_count][
                ...
                for sd in sd_schedules:
                    ...
                    if "child" in sd:
                        ...
                        for sd_child in sd_children:
                            if sd_child.get("transactionTypeIdentifier") in [
                                "OPEXP_DEBT",
                                "FEA_100PCT_DEBT_PAY",
                                "OTH_DISB_DEBT",
                            ]:
        ...
        if "SA" in schedules:
            ...
            if sa_schedules:
                ...
                sa_11a_last_page_cnt = (
                    sa_11b_last_page_cnt
                ) = sa_11c_last_page_cnt = sa_12_last_page_cnt = 3
                sa_13_last_page_cnt = (
                    sa_14_last_page_cnt
                ) = sa_15_last_page_cnt = sa_16_last_page_cnt = 3
                sa_17_last_page_cnt = 3

                sa_11a_memo_last_page_cnt = (
                    sa_11b_memo_last_page_cnt
                ) = sa_11c_memo_last_page_cnt = sa_12_memo_last_page_cnt = 2
                sa_13_memo_last_page_cnt = (
                    sa_14_memo_last_page_cnt
                ) = sa_15_memo_last_page_cnt = sa_16_memo_last_page_cnt = 2
                ...
                sa_17_page_cnt = (
                    sa_11a_memo_page_cnt
                ) = (
                    sa_11b_memo_page_cnt
                ) = sa_11c_memo_page_cnt = sa_12_memo_page_cnt = 0
                sa_13_memo_page_cnt = (
                    sa_14_memo_page_cnt
                ) = sa_15_memo_page_cnt = sa_16_memo_page_cnt = 0
                ...
                (
                    sa_11a_memo_page_cnt,
                    sa_11a_memo_last_page_cnt,
                ) = calculate_memo_page_count(sa_11a_memo)
                (
                    sa_11b_memo_page_cnt,
                    sa_11b_memo_last_page_cnt,
                ) = calculate_memo_page_count(sa_11b_memo)
                (
                    sa_11c_memo_page_cnt,
                    sa_11c_memo_last_page_cnt,
                ) = calculate_memo_page_count(sa_11c_memo)
                (
                    sa_12_memo_page_cnt,
                    sa_12_memo_last_page_cnt,
                ) = calculate_memo_page_count(sa_12_memo)
                (
                    sa_13_memo_page_cnt,
                    sa_13_memo_last_page_cnt,
                ) = calculate_memo_page_count(sa_13_memo)
                (
                    sa_14_memo_page_cnt,
                    sa_14_memo_last_page_cnt,
                ) = calculate_memo_page_count(sa_14_memo)
                (
                    sa_15_memo_page_cnt,
                    sa_15_memo_last_page_cnt,
                ) = calculate_memo_page_count(sa_15_memo)
                (
                    sa_16_memo_page_cnt,
                    sa_16_memo_last_page_cnt,
                ) = calculate_memo_page_count(sa_16_memo)
                (
                    sa_17_memo_page_cnt,
                    sa_17_memo_last_page_cnt,
                ) = calculate_memo_page_count(sa_17_memo)
        ...
        if "SB" in schedules or len(sb_schedules) > 0:
            ...
            if sb_schedules_cnt > 0:
                ...
                sb_21b_last_page_cnt = (
                    sb_22_last_page_cnt
                ) = sb_23_last_page_cnt = sb_26_last_page_cnt = 3
                sb_27_last_page_cnt = (
                    sb_28a_last_page_cnt
                ) = sb_28b_last_page_cnt = sb_28c_last_page_cnt = 3
                sb_29_last_page_cnt = sb_30b_last_page_cnt = 3

                sb_21b_memo_last_page_cnt = (
                    sb_22_memo_last_page_cnt
                ) = sb_23_memo_last_page_cnt = sb_26_memo_last_page_cnt = 2
                sb_27_memo_last_page_cnt = (
                    sb_28a_memo_last_page_cnt
                ) = sb_28b_memo_last_page_cnt = sb_28c_memo_last_page_cnt = 2
                ...
                sb_21b_memo_page_cnt = (
                    sb_22_memo_page_cnt
                ) = sb_23_memo_page_cnt = sb_26_memo_page_cnt = 0
                sb_27_memo_page_cnt = (
                    sb_28a_memo_page_cnt
                ) = sb_28b_memo_page_cnt = sb_28c_memo_page_cnt = 0
                sb_29_memo_page_cnt = sb_30b_memo_page_cnt = 0

                # process for each Schedule B
                for sb_count in range(sb_schedules_cnt):
                    process_sb_line_numbers(
                        sb_21b,
                        sb_22,
                        sb_23,
                        sb_26,
                    ...
                    if "child" in sb_schedules[sb_count]:
                        ...
                        for sb_child_count in range(sb_child_schedules_count):
                            if (
                                sb_schedules[sb_count]["child"][sb_child_count][
                                    "lineNumber"
                                ]
                                in sb_line_numbers
                            ):
                ...
                (
                    sb_21b_memo_page_cnt,
                    sb_21b_memo_last_page_cnt,
                ) = calculate_memo_page_count(sb_21b_memo)
                (
                    sb_22_memo_page_cnt,
                    sb_22_memo_last_page_cnt,
                ) = calculate_memo_page_count(sb_22_memo)
                (
                    sb_23_memo_page_cnt,
                    sb_23_memo_last_page_cnt,
                ) = calculate_memo_page_count(sb_23_memo)
                (
                    sb_26_memo_page_cnt,
                    sb_26_memo_last_page_cnt,
                ) = calculate_memo_page_count(sb_26_memo)
                (
                    sb_27_memo_page_cnt,
                    sb_27_memo_last_page_cnt,
                ) = calculate_memo_page_count(sb_27_memo)
                (
                    sb_28a_memo_page_cnt,
                    sb_28a_memo_last_page_cnt,
                ) = calculate_memo_page_count(sb_28a_memo)
                (
                    sb_28b_memo_page_cnt,
                    sb_28b_memo_last_page_cnt,
                ) = calculate_memo_page_count(sb_28b_memo)
                (
                    sb_28c_memo_page_cnt,
                    sb_28c_memo_last_page_cnt,
                ) = calculate_memo_page_count(sb_28c_memo)
                (
                    sb_29_memo_page_cnt,
                    sb_29_memo_last_page_cnt,
                ) = calculate_memo_page_count(sb_29_memo)
                (
                    sb_30b_memo_page_cnt,
                    sb_30b_memo_last_page_cnt,
                ) = calculate_memo_page_count(sb_30b_memo)
        ...
        if "SE" in schedules or len(se_schedules) > 0:
            ...
            if se_schedules_cnt > 0:
                ...
                for se_count in range(se_schedules_cnt):
                    ...
                    if "child" in se_schedules[se_count]:
                        ...
                        for se_child_count in range(se_child_schedules_count):
                            if (
                                se_schedules[se_count]["child"][se_child_count][
                                    "lineNumber"
                                ]
                                in se_line_numbers
                            ):
                                process_se_line_numbers(
                                    se_24,
                                    se_24_memo,
                                    se_schedules[se_count]["child"][se_child_count],
                                )
                ...
                (
                    se_24_memo_page_cnt,
                    se_24_memo_last_page_cnt,
                ) = calculate_memo_page_count(se_24_memo)

                total_no_of_pages = (
                    total_no_of_pages + se_24_page_cnt + se_24_memo_page_cnt
                )
        ...
        if "SF" in schedules:
            ...
            if sf_schedules:
                ...
                sf_crd_last_page_cnt = (
                    sf_non_crd_last_page_cnt
                ) = sf_empty_ord_last_page_cnt = 3
                sf_empty_non_ord_last_page_cnt = (
                    sf_empty_none_last_page_cnt
                ) = sf_non_sub_last_page_cnt = 3
                sf_crd_memo_last_page_cnt = (
                    sf_non_crd_memo_last_page_cnt
                ) = sf_empty_ord_memo_last_page_cnt = 2
                sf_empty_non_ord_memo_last_page_cnt = (
                    sf_empty_none_memo_last_page_cnt
                ) = sf_non_sub_memo_last_page_cnt = 2

                sf_crd_page_cnt = (
                    sf_non_crd_page_cnt
                ) = sf_empty_ord_page_cnt = sf_empty_non_ord_page_cnt = 0
                sf_empty_none_page_cnt = (
                    sf_non_sub_page_cnt
                ) = sf_crd_memo_page_cnt = sf_non_crd_memo_page_cnt = 0
                ...
                for lis in list_dirs:
                    ...
                    for val in values:
                        sf_crd_page_cnt, sf_crd_last_page_cnt = calculate_page_count(
                            val
                        )
        ...
        if "SH" in schedules:
            ...
            if sh_schedules_cnt > 0:
                ...
                sh_30a_memo_last_page_cnt = (
                    sh_21a_memo_last_page_cnt
                ) = sh_18b_memo_last_page_cnt = 2
                sh_30a_page_cnt = (
                    sh_21a_page_cnt
                ) = sh_18b_page_cnt = sh_30a_memo_page_cnt = 0
                ...
                for sh_count in range(sh_schedules_cnt):
                    ...
                    (
                        sh_30a_memo_page_cnt,
                        sh_30a_memo_last_page_cnt,
                    ) = calculate_memo_page_count(sh_30a_memo)

                    sh_21a_page_cnt, sh_21a_last_page_cnt = calculate_page_count(sh_21a)
                    total_no_of_pages = total_no_of_pages + sh_21a_page_cnt
                    (
                        sh_21a_memo_page_cnt,
                        sh_21a_memo_last_page_cnt,
                    ) = calculate_memo_page_count(sh_21a_memo)

                    sh_18b_page_cnt, sh_18b_last_page_cnt = calculate_sh5page_count(
                        sh_18b
                    )
                    total_no_of_pages = total_no_of_pages + sh_18b_page_cnt
                    (
                        sh_18b_memo_page_cnt,
                        sh_18b_memo_last_page_cnt,
                    ) = calculate_memo_page_count(sh_18b_memo)

                    sh_18a_page_cnt, sh_18a_last_page_cnt = calculate_page_count(sh_18a)
                    total_no_of_pages = total_no_of_pages + sh_18a_page_cnt
                    (
                        sh_18a_memo_page_cnt,
                        sh_18a_memo_last_page_cnt,
                    ) = calculate_memo_page_count(sh_18a_memo)
        ...
        if "SL-A" in schedules:
            ...
            if la_schedules:
                ...
                la_1a_page_cnt = (
                    la_2_page_cnt
                ) = la_1a_memo_page_cnt = la_2_memo_page_cnt = 0

                la_schedules_cnt = len(la_schedules)

                for la_count in range(la_schedules_cnt):
                    process_la_line_numbers(
                        la_1a, la_2, la_1a_memo, la_2_memo, la_schedules[la_count]
                    )
                ...
                la_1a_memo_page_cnt, la_1a_memo_last_page_cnt = calculate_la_page_count(
                    la_1a_memo
                )
                la_2_memo_page_cnt, la_2_memo_last_page_cnt = calculate_la_page_count(
                    la_2_memo
                )
        ...
        if "SL-B" in schedules or len(slb_schedules) > 0:
            ...
            if slb_schedules_cnt > 0:
                ...
                slb_4a_last_page_cnt = (
                    slb_4b_last_page_cnt
                ) = (
                    slb_4c_last_page_cnt
                ) = slb_4d_last_page_cnt = slb_5_last_page_cnt = 5
                slb_4a_memo_last_page_cnt = (
                    slb_4b_memo_last_page_cnt
                ) = slb_4c_memo_last_page_cnt = 2
                slb_4d_memo_last_page_cnt = slb_5_memo_last_page_cnt = 2

                slb_4a_page_cnt = (
                    slb_4b_page_cnt
                ) = slb_4c_page_cnt = slb_4d_page_cnt = lb_5_page_cnt = 0
                slb_4a_memo_page_cnt = (
                    slb_4b_memo_page_cnt
                ) = slb_4c_memo_page_cnt = slb_4d_memo_page_cnt = lb_5_memo_page_cnt = 0

                for slb_count in range(slb_schedules_cnt):
                    process_slb_line_numbers(
                        slb_4a,
                        slb_4b,
                        slb_4c,
                        slb_4d,
                ...
                (
                    slb_4a_memo_page_cnt,
                    slb_4a_memo_last_page_cnt,
                ) = calculate_slb_page_count(slb_4a_memo)
                (
                    slb_4b_memo_page_cnt,
                    slb_4b_memo_last_page_cnt,
                ) = calculate_slb_page_count(slb_4b_memo)
                (
                    slb_4c_memo_page_cnt,
                    slb_4c_memo_last_page_cnt,
                ) = calculate_slb_page_count(slb_4c_memo)
                (
                    slb_4d_memo_page_cnt,
                    slb_4d_memo_last_page_cnt,
                ) = calculate_slb_page_count(slb_4d_memo)
                (
                    slb_5_memo_page_cnt,
                    slb_5_memo_last_page_cnt,
                ) = calculate_slb_page_count(slb_5_memo)
        ...
        if "SC" in schedules and sc_schedules_cnt > 0:
            sc1_list, sc1_start_page, totalOutstandingLoans = process_sc_line(
                f3x_data, md5_directory, sc_schedules, sc_start_page, total_no_of_pages
            )
        else:
            sc1_list = []
        ...
        if sc1_list:
            for sc1 in sc1_list:
                process_sc1_line(
                    f3x_data, md5_directory, sc1, sc1_start_page, total_no_of_pages
                )
                sc1_start_page += 1

        if "SD" in schedules and sd_schedules_cnt > 0:
            sd_total_balance = process_sd_line(
                f3x_data,
                md5_directory,
                sd_dict,
                sd_start_page,
    ...
    for line_number in sd_dict:
        ...
        if sd_list:
            for i in range(len(sd_list)):
                ...
                concat_no = i % 3 + 1
                if (
                    "creditorOrganizationName" in sd_list[i]
                    and sd_list[i].get("creditorOrganizationName") != ""
                ):
                    sd_page_dict["creditorName_{}".format(concat_no)] = sd_list[i].get(
                ...
                for item in [
                    ...
                    sd_page_dict[item + "_{}".format(concat_no)] = "{0:.2f}".format(
                        float(sd_list[i].get(item))
                    )
                if i % 3 == 2 or i == len(sd_list) - 1:
                    ...
                    if page_count == total_sd_pages - 1:
                        ...
                        sd_total_balance = sd_page_dict[
                            ...
                            sd_schedule_total + float(totalOutstandingLoans)
                        )
                    sd_outfile = (
                        md5_directory
                        + "SD"
                        + "/page_"
                        + str(sd_page_dict["pageNo"])
                        + ".pdf"
                    )
            ...
            if sc2:
                ...
                for i in range(len(sc2_list_list)):
                    ...
                    for j in range(len(sc2_list_list[i])):
                        ...
                        sc_schedule_single_page_dict["name{}".format(j + 1)] = sc2_name[
                            0:-1
                        ]
                        sc_schedule_single_page_dict[
                            "street1_{}".format(j + 1)
                        ] = sc2_list_list[i][j].get("street1")
                        sc_schedule_single_page_dict[
                            "street2_{}".format(j + 1)
                        ] = sc2_list_list[i][j].get("street2")
                        sc_schedule_single_page_dict[
                            "city_{}".format(j + 1)
                        ] = sc2_list_list[i][j].get("city")
                        sc_schedule_single_page_dict[
                            "state_{}".format(j + 1)
                        ] = sc2_list_list[i][j].get("state")
                        sc_schedule_single_page_dict[
                            "zipCode_{}".format(j + 1)
                        ] = sc2_list_list[i][j].get("zipCode")
                        sc_schedule_single_page_dict[
                            "employer_{}".format(j + 1)
                        ] = sc2_list_list[i][j].get("employer")
                        sc_schedule_single_page_dict[
                            "occupation_{}".format(j + 1)
                        ] = sc2_list_list[i][j].get("occupation")
                        sc_schedule_single_page_dict[
                            "guaranteedAmount_{}".format(j + 1)
                        ] = "{0:.2f}".format(
                            float(sc2_list_list[i][j].get("guaranteedAmount"))
                        )
                    ...
                    # Memo text changes
                    if (
                        "memoDescription" in sc_schedule_page_dict
                        and sc_schedule_page_dict["memoDescription"]
                    ):
                        memo_dict = {}
...
def process_sc1_line(f3x_data, md5_directory, sc1, sc1_start_page, total_no_of_pages):
    ...
    # Memo text changes
    if (
        "memoDescription" in sc1_schedule_page_dict
        and sc1_schedule_page_dict["memoDescription"]
    ):
        memo_dict = {}
        temp_memo_outfile = md5_directory + "SC/page_memo_temp.pdf"
    ...
    if len(sa_line) > 0:
        ...
        if sa_line_page_cnt > 0:
            for sa_page_no in range(sa_line_page_cnt):
                ...
                sa_schedule_dict = build_sa_per_page_schedule_dict(
                    last_page,
                    sa_line_last_page_cnt,
                    page_start_index,
                    sa_schedule_page_dict,
                ...
                if sa_line_page_cnt == (sa_page_no + 1):
                    sa_schedule_page_dict["scheduleTotal"] = "{0:.2f}".format(
                        schedule_total
                    )
                ...
                if len(memo_array) >= 1:
                    temp_memo_outfile = (
                        md5_directory + "SA/" + line_number + "/page_memo_temp.pdf"
                    )
                    memo_infile = current_app.config["FORM_TEMPLATES_LOCATION"].format(
                        "TEXT"
                    )
        ...
        if sb_line_page_cnt > 0:
            ...
            for sb_page_no in range(sb_line_page_cnt):
                ...
                if (sb_page_no + 1) == sb_line_page_cnt:
                    last_page = True
                # This call prepares data to render on PDF
                sb_schedule_dict = build_sb_per_page_schedule_dict(
                    last_page,
                    sb_line_last_page_cnt,
                    page_start_index,
                    sb_schedule_page_dict,
                ...
                if sb_line_page_cnt == (sb_page_no + 1):
                    sb_schedule_page_dict["scheduleTotal"] = "{0:.2f}".format(
                        schedule_total
                    )
                ...
                if len(memo_array) >= 1:
                    temp_memo_outfile = (
                        md5_directory + "SB/" + line_number + "/page_memo_temp.pdf"
                    )
                    memo_infile = current_app.config["FORM_TEMPLATES_LOCATION"].format(
                        "TEXT"
                    )
        ...
        if se_line_page_cnt > 0:
            ...
            for se_page_no in range(se_line_page_cnt):
                ...
                if (se_page_no + 1) == se_line_page_cnt:
                    last_page = True
                # This call prepares data to render on PDF
                se_schedule_dict = build_se_per_page_schedule_dict(
                    last_page,
                    se_line_last_page_cnt,
                    page_start_index,
                    se_schedule_page_dict,
                ...
                if se_line_page_cnt == (se_page_no + 1):
                    se_schedule_page_dict["pageTotal"] = "{0:.2f}".format(
                        schedule_total
                    )
        ...
        if sf_line_page_cnt > 0:
            ...
            for sf_page_no in range(sf_line_page_cnt):
                ...
                if (sf_page_no + 1) == sf_line_page_cnt:
                    last_page = True
                # This call prepares data to render on PDF
                sf_schedule_dict = build_sf_per_page_schedule_dict(
                    last_page,
                    sf_line_last_page_cnt,
                    page_start_index,
                    sf_schedule_page_dict,
                ...
                if sf_line_page_cnt == (sf_page_no + 1):
                    sf_schedule_page_dict["pageTotal"] = "{0:.2f}".format(
                        schedule_total
                    )
                ...
                if len(memo_array) >= 1:
                    temp_memo_outfile = (
                        md5_directory + "SF/" + cord_name + "/page_memo_temp.pdf"
                    )
                    memo_infile = current_app.config["FORM_TEMPLATES_LOCATION"].format(
                        "TEXT"
                    )
        ...
        if len(la_line) > 0:
            ...
            if la_line_page_cnt > 0:
                for la_page_no in range(la_line_page_cnt):
                    ...
                    la_schedule_dict = build_la_per_page_schedule_dict(
                        last_page,
                        la_line_last_page_cnt,
                        page_start_index,
                        la_schedule_page_dict,
                    ...
                    if la_line_page_cnt == (la_page_no + 1):
                        la_schedule_page_dict["scheduleTotal"] = "{0:.2f}".format(
                            schedule_total
                        )
                    ...
                    if len(memo_array) >= 1:
                        ...
                        if len(memo_array) >= 3:
                            ...
                            pypdftk.fill_form(memo_infile, memo_dict, memo_outfile)
                            pypdftk.concat(
                                [la_outfile, memo_outfile], temp_memo_outfile
                            )
                            os.remove(memo_outfile)
                            os.rename(temp_memo_outfile, la_outfile)
            pypdftk.concat(
        ...
        if slb_line_page_cnt > 0:
            ...
            for slb_page_no in range(slb_line_page_cnt):
                ...
                if (slb_page_no + 1) == slb_line_page_cnt:
                    last_page = True
                # This call prepares data to render on PDF
                slb_schedule_dict = build_slb_per_page_schedule_dict(
                    last_page,
                    slb_line_last_page_cnt,
                    page_start_index,
                    slb_schedule_page_dict,
                ...
                if slb_line_page_cnt == (slb_page_no + 1):
                    slb_schedule_page_dict["scheduleTotal"] = "{0:.2f}".format(
                        schedule_total
                    )
...
def process_slb_memo(
    ...
    try:
        if len(slb_memo) > 0:
            ...
            slb_memo_infile = current_app.config["FORM_TEMPLATES_LOCATION"].format(
                "TEXT"
            )
            if slb_memo_page_cnt > 0:
                for slb_memo_page_no in range(slb_memo_page_cnt):
                    ...
                    slb_memo_schedule_page_dict["PAGESTR"] = (
                        "PAGE "
                        + str(slb_memo_page_no + 1)
                        + " / "
                    ...
                    if (slb_memo_page_no + 1) == slb_memo_page_cnt:
                        last_page = True
                    # This call prepares data to render on PDF
                    slb_memo_schedule_page_dict = build_memo_per_page_schedule_dict(
                        last_page,
                        slb_memo_last_page_cnt,
                        page_start_index,
                        slb_memo_schedule_page_dict,
                    ...
                    slb_memo_outfile = (
                        ...
                        + line_number
                        + "/memo/page_"
                        + str(slb_memo_page_no)
                        + ".pdf"
                    )
                    pypdftk.fill_form(
                        slb_memo_infile, slb_memo_schedule_page_dict, slb_memo_outfile
                    )
                pypdftk.concat(
                    directory_files(md5_directory + "SL-B/" + line_number + "/memo/"),
            ...
            if sl_line_page_cnt > 0:
                for sl_page_no in range(sl_line_page_cnt):
                    ...
                    if (sl_page_no + 1) == sl_line_page_cnt:
                        last_page = True
                    sl_schedule_dict = build_sl_levin_per_page_schedule_dict(
                        last_page,
                        sl_line_last_page_cnt,
                        page_start_index,
                        sl_schedule_page_dict,
        ...
        if sh1_page_cnt > 0:
            ...
            for sh1_line in sh_h1:
                ...
                if (
                    ...
                    sh1_schedule_page_dict["nonPresidentialAndNonSenate"] = str(
                        sh1_line["nonPresidentialAndNonSenate"]
                    )
                else:
                    sh1_schedule_page_dict["federalPercent"] = "{0:.2f}".format(
                        float(sh1_line["federalPercent"])
                    )
                    sh1_schedule_page_dict["nonFederalPercent"] = "{0:.2f}".format(
        ...
        if sh2_line_page_cnt > 0:
            ...
            for sh2_page_no in range(sh2_line_page_cnt):
                ...
                if (sh2_page_no + 1) == sh2_line_page_cnt:
                    last_page = True
                    # This call prepares data to render on PDF
                sh2_schedule_dict = build_sh2_per_page_schedule_dict(
                    last_page,
                    sh2_line_last_page_cnt,
                    page_start_index,
                    sh2_schedule_page_dict,
        ...
        if sh6_line_page_cnt > 0:
            ...
            for sh6_page_no in range(sh6_line_page_cnt):
                ...
                if (sh6_page_no + 1) == sh6_line_page_cnt:
                    last_page = True
                # This call prepares data to render on PDF
                sh6_schedule_dict = build_sh6_line_per_page_schedule_dict(
                    last_page,
                    sh6_line_last_page_cnt,
                    page_start_index,
                    sh6_schedule_page_dict,
                ...
                page_fed_subtotal = float(
                    sh6_schedule_page_dict["subTotalFederalShare"]
                )
                page_levin_subtotal = float(
                    sh6_schedule_page_dict["subTotalLevinShare"]
                )

                sh6_schedule_page_dict["fedLevinSubTotalShare"] = (
                    page_fed_subtotal + page_levin_subtotal
                )

                total_federal_share += page_fed_subtotal
                total_levin_share += page_levin_subtotal
                if sh6_line_page_cnt == (sh6_page_no + 1):
                    sh6_schedule_page_dict["totalFederalShare"] = "{0:.2f}".format(
                        total_federal_share
                    )
                    sh6_schedule_page_dict["totallevinShare"] = "{0:.2f}".format(
                        total_levin_share
                    )
                    sh6_schedule_page_dict["fedLevinTotalShare"] = (
                        total_federal_share + total_levin_share
                    )
                ...
                if len(memo_array) >= 1:
                    temp_memo_outfile = (
                        md5_directory + "SH6/" + line_number + "/page_memo_temp.pdf"
                    )
                    memo_infile = current_app.config["FORM_TEMPLATES_LOCATION"].format(
                        "TEXT"
                    )
                    ...
                    if len(memo_array) >= 3:
                        ...
                        os.rename(temp_memo_outfile, sh6_outfile)
        pypdftk.concat(
            directory_files(md5_directory + "SH6/" + line_number + "/"),
            md5_directory + "SH6/" + line_number + "/all_pages.pdf",
        )
        ...
        if sh4_line_page_cnt > 0:
            ...
            for sh4_page_no in range(sh4_line_page_cnt):
                ...
                if (sh4_page_no + 1) == sh4_line_page_cnt:
                    last_page = True
                # This call prepares data to render on PDF
                sh4_schedule_dict = build_sh4_per_page_schedule_dict(
                    last_page,
                    sh4_line_last_page_cnt,
                    page_start_index,
                    sh4_schedule_page_dict,
                ...
                sh4_schedule_page_dict["subTotalFedNonFedShare"] = "{0:.2f}".format(
                ...
                if sh4_line_page_cnt == (sh4_page_no + 1):
                    sh4_schedule_page_dict["TotalFedShare"] = "{0:.2f}".format(
                        total_fedshare
                    )
                    sh4_schedule_page_dict["totalNonFedShare"] = "{0:.2f}".format(
                        total_nonfedshare
                    )
                    sh4_schedule_page_dict["TotalFedNonFedShare"] = "{0:.2f}".format(
                ...
                if len(memo_array) >= 1:
                    temp_memo_outfile = (
                        md5_directory + "SH4/" + line_number + "/page_memo_temp.pdf"
                    )
                    memo_infile = current_app.config["FORM_TEMPLATES_LOCATION"].format(
                        "TEXT"
                    )
        ...
        if sh5_line_page_cnt > 0:
            ...
            for sh5_page_no in range(sh5_line_page_cnt):
                ...
                if (sh5_page_no + 1) == sh5_line_page_cnt:
                    last_page = True
                # This call prepares data to render on PDF
                sh5_schedule_dict = build_sh5_per_page_schedule_dict(
                    last_page,
                    sh5_line_last_page_cnt,
                    page_start_index,
                    sh5_schedule_page_dict,
                ...
                voter_reg_amt_subtotal = float(
                    sh5_schedule_page_dict["subvoterRegistrationAmount"]
                )
                voter_id_amt_subtotal = float(
                    sh5_schedule_page_dict["subvoterIdAmount"]
                )
                gotv_amt_subtotal = float(sh5_schedule_page_dict["subgotvAmount"])
                generic_camp_amt_subtotal = float(
                    sh5_schedule_page_dict["subgenericCampaignAmount"]
                )
                ...
                if sh5_line_page_cnt == (sh5_page_no + 1):
                    sh5_schedule_page_dict[
                        "totalvoterRegistrationAmount"
                    ] = "{0:.2f}".format(total_voter_reg_amt_subtotal)
                    sh5_schedule_page_dict["totalvoterIdAmount"] = "{0:.2f}".format(
                        total_voter_id_amt_subtotal
                    )
                    sh5_schedule_page_dict["totalgotvAmount"] = "{0:.2f}".format(
                        total_gotv_amt_subtotal
                    )
                    sh5_schedule_page_dict[
                        "totalgenericCampaignAmount"
                    ] = "{0:.2f}".format(total_generic_camp_amt_subtotal)
                    sh5_schedule_page_dict["totalAmountOfTransfersReceived"] = (
...
def process_sa_line_numbers(
    ...
    if (
        sa_obj["lineNumber"] == "11A"
        or sa_obj["lineNumber"] == "11AI"
        or sa_obj["lineNumber"] == "11AII"
    ):
        sa_11a.append(sa_obj)
        if sa_obj["memoDescription"]:
            sa_11a_memo.append(
                {
                    "scheduleName": "SA" + sa_obj["lineNumber"],
                    "memoDescription": sa_obj["memoDescription"],
            ...
            sb_21b_memo.append(
                {
                    ...
                    "memoDescription": sb_obj["memoDescription"],
...
def process_se_line_numbers(se_24, se_24_memo, se_obj):
    if se_obj["lineNumber"] == "24":
        ...
        if se_obj["memoDescription"]:
            se_24_memo.append(
                {
                    "scheduleName": "SE" + se_obj["lineNumber"],
                    "memoDescription": se_obj["memoDescription"],
                    "transactionId": se_obj["transactionId"],
                }
            )


def process_sf_line_numbers(
    sf_crd,
    sf_non_crd,
    sf_empty_ord,
    sf_empty_non_ord,
    ...
    if sf_obj["coordinateExpenditure"] == "Y":
        ...
        if sf_obj["memoDescription"]:
            sf_crd_memo.append(
                {
                    "scheduleName": "SF" + sf_obj["lineNumber"],
                    "memoDescription": sf_obj["memoDescription"],
        ...
        sf_obj["coordinateExpenditure"] == ""
        and sf_obj["subordinateCommitteeName"] == ""
        and sf_obj["designatingCommitteeName"] == ""
    ):
        ...
        sf_obj["coordinateExpenditure"] == "N"
        and sf_obj["subordinateCommitteeName"] == ""
    ):
...
def process_la_line_numbers(la_1a, la_2, la_1a_memo, la_2_memo, la_obj):
    if la_obj["lineNumber"] == "1A":
        ...
        if la_obj["memoDescription"]:
            la_1a_memo.append(
                {
                    "scheduleName": "SL" + la_obj["lineNumber"],
                    "memoDescription": la_obj["memoDescription"],
...
def process_slb_line_numbers(
    ...
    if slb_obj["lineNumber"] == "4A":
        ...
        if slb_obj["memoDescription"]:
            slb_4a_memo.append(
                {
                    "scheduleName": "SL" + slb_obj["lineNumber"],
                    "memoDescription": slb_obj["memoDescription"],
...
def process_sh_line_numbers(
    ...
    if sh_obj["lineNumber"] == "30A":
        ...
        if "memoDescription" in sh_obj and sh_obj["memoDescription"]:
            sh_30a_memo.append(
                {
                    "scheduleName": "SH" + sh_obj["lineNumber"],
                    "memoDescription": sh_obj["memoDescription"],
...
def build_sa_per_page_schedule_dict(
    last_page,
    ...
    if transactions_in_page == 1:
        ...
        build_contributor_name_date_dict(
...
def build_memo_per_page_schedule_dict(
    last_page,
    transactions_in_page,
    page_start_index,
    memo_schedule_page_dict,
    memo_schedules,
):
    if not last_page:
    ...
    if transactions_in_page == 1:
        ...
        build_memo_dict(
            index, page_start_index, memo_schedule_dict, memo_schedule_page_dict
        )
    elif transactions_in_page == 2:
        index = 1
        memo_schedule_dict = memo_schedules[page_start_index + 0]
...
def build_sh4_per_page_schedule_dict(
    ...
    if transactions_in_page == 1:
        ...
        build_sh_name_date_dict(
            index, page_start_index, sh4_schedule_dict, sh4_schedule_page_dict
        )
    ...
    sh4_schedule_page_dict["subTotalFedNonFedShare"] = float(
        sh4_schedule_page_dict["subFedShare"]
    ) + float(sh4_schedule_page_dict["subNonFedShare"])
...
def build_sh2_per_page_schedule_dict(
    ...
    if transactions_in_page == 1:
        ...
        for key in sh2_schedules[page_start_index]:
            build_sh2_name_date_dict(
                index, key, sh2_schedule_dict, sh2_schedule_page_dict
            )
    elif transactions_in_page == 2:
        index = 1
        sh2_schedule_dict = sh2_schedules[page_start_index + 0]
...
def build_sh5_per_page_schedule_dict(
    last_page,
    transactions_in_page,
    page_start_index,
    sh5_schedule_page_dict,
    ...
    sh5_schedule_page_dict["subtotalAmountTransferred"] = "{0:.2f}".format(
        transferred_amt_subtotal
    )
    sh5_schedule_page_dict["subvoterRegistrationAmount"] = "{0:.2f}".format(
        voter_reg_amt_subtotal
    )
    ...
    sh5_schedule_page_dict["subgenericCampaignAmount"] = "{0:.2f}".format(
...
def build_sh6_line_per_page_schedule_dict(
    last_page,
    transactions_in_page,
    page_start_index,
    sh6_schedule_page_dict,
    ...
    if transactions_in_page == 1:
        ...
        build_sh_name_date_dict(
            index, page_start_index, sh6_schedule_dict, sh6_schedule_page_dict
        )
    ...
    sh6_schedule_page_dict["fedLevinSubTotalShare"] = float(
        sh6_schedule_page_dict["subTotalFederalShare"]
    ) + float(sh6_schedule_page_dict["subTotalLevinShare"])
    ...
    if transactions_in_page == 1:
        ...
        for key in sb_schedules[page_start_index]:
            build_payee_name_date_dict(
                index, key, sb_schedule_dict, sb_schedule_page_dict
            )
    elif transactions_in_page == 2:
        index = 1
        sb_schedule_dict = sb_schedules[page_start_index + 0]
    ...
    if transactions_in_page == 1:
        ...
        for key in sf_schedules[page_start_index]:
            build_payee_sf_name_date_dict(
                index, key, sf_schedule_dict, sf_schedule_page_dict
            )
    elif transactions_in_page == 2:
        index = 1
        sf_schedule_dict = sf_schedules[page_start_index + 0]
    ...
    try:
        ...
        if tranlactions_in_page == 1:
            ...
            build_contributor_la_name_date_dict(
                index, page_start_index, la_schedule_dict, la_schedule_page_dict
            )
...
def build_contributor_name_date_dict(
    index, key, sa_schedule_dict, sa_schedule_page_dict
):
    try:
        if (
            "contributorLastName" in sa_schedule_dict
            and sa_schedule_dict["contributorLastName"]
        ):
            sa_schedule_page_dict["contributorName_" + str(index)] = (
                sa_schedule_dict["contributorLastName"]
                + ","
            ...
            sa_schedule_page_dict["contributorName_" + str(index)] = sa_schedule_dict[
                "contributorOrgName"
            ]
        ...
        if "contributionAmount" in sa_schedule_dict:
            ...
            sa_schedule_page_dict[
                "contributionAmount_" + str(index)
            ] = "{0:.2f}".format(sa_schedule_dict["contributionAmount"])
        ...
        if "contributionAggregate" in sa_schedule_dict:
            ...
            ] = "{0:.2f}".format(sa_schedule_dict["contributionAggregate"])
        ...
        print(
            "Error at key: "
            + key
            + " in Schedule A transaction: "
            + str(sa_schedule_dict)
        )
        raise e
...
def build_memo_dict(index, key, sa_memo_schedule_dict, sa_memo_schedule_page_dict):
    try:
        for key in sa_memo_schedule_dict:
            # if key != 'lineNumber':
            sa_memo_schedule_page_dict[key + "_" + str(index)] = sa_memo_schedule_dict[
                key
            ]
    except Exception as e:
        print(
            "Error at key: "
            + key
            + " in Schedule A memo transaction: "
            + str(sa_memo_schedule_dict)
        )
        raise e
...
def build_payee_name_date_dict(index, key, sb_schedule_dict, sb_schedule_page_dict):
    try:
        ...
        if "payeeLastName" in sb_schedule_dict and sb_schedule_dict["payeeLastName"]:
            sb_schedule_page_dict["payeeName_" + str(index)] = (
                sb_schedule_dict["payeeLastName"]
                + ","
        ...
        if (
            "beneficiaryCandidateLastName" in sb_schedule_dict
            and sb_schedule_dict["beneficiaryCandidateLastName"]
        ):
        ...
        if key == "electionCode":
            if sb_schedule_dict[key] and sb_schedule_dict[key][0] in ["P", "G"]:
                sb_schedule_page_dict["electionType_" + str(index)] = sb_schedule_dict[
                    key
                ][0:1]
            else:
                sb_schedule_page_dict["electionType_" + str(index)] = "O"
            sb_schedule_page_dict["electionYear_" + str(index)] = sb_schedule_dict[key][
                1::
            ]
        ...
        if key == "expenditureDate":
            ...
            if key == "expenditureAmount" or key == "expenditureAggregate":
                sb_schedule_page_dict[key + "_" + str(index)] = "{0:.2f}".format(
                    sb_schedule_dict[key]
                )
            else:
        ...
        print(
            "Error at key: "
            + key
            + " in Schedule B transaction: "
            + str(sb_schedule_dict)
        )
        raise e
...
def build_se_name_date_dict(index, key, se_schedule_dict, se_schedule_page_dict):
    try:
        ...
        if "payeeLastName" in se_schedule_dict and se_schedule_dict["payeeLastName"]:
            se_schedule_page_dict["payeeName_" + str(index)] = (
                se_schedule_dict["payeeLastName"]
                + ","
            ...
            se_schedule_page_dict["payeeName_" + str(index)] = se_schedule_dict[
                "payeeOrganizationName"
            ]

        if "candidateLastName" in se_schedule_dict:
            se_schedule_page_dict["candidateName_" + str(index)] = (
                se_schedule_dict["candidateLastName"]
                + ","
        ...
        if "completingLastName" in se_schedule_dict:
            se_schedule_page_dict["completingName_" + str(index)] = (
                se_schedule_dict["completingLastName"]
                + ","
        ...
        if key == "electionCode":
            if se_schedule_dict[key][0] in ["P", "G"]:
                se_schedule_page_dict["electionType_" + str(index)] = se_schedule_dict[
                    key
                ][0:1]
            else:
                se_schedule_page_dict["electionType_" + str(index)] = "O"
            se_schedule_page_dict["electionYear_" + str(index)] = se_schedule_dict[key][
                1::
            ]

        if (key == "disseminationDate" and len(se_schedule_dict[key])) > 0:
            date_array = se_schedule_dict[key].split("/")
            se_schedule_page_dict["disseminationDateMonth_" + str(index)] = date_array[
                0
            ]
            ...
            if key == "expenditureAmount" or key == "calendarYTDPerElectionForOffice":
                se_schedule_page_dict[key + "_" + str(index)] = (
                    "{0:.2f}".format(se_schedule_dict[key])
                    if se_schedule_dict[key]
                    else 0.0
                )
            else:
        ...
        print(
            "Error at key: "
            + key
            + " in Schedule E transaction: "
            + str(se_schedule_dict)
        )
        raise e
...
def build_payee_sf_name_date_dict(index, key, sf_schedule_dict, sf_schedule_page_dict):
    try:
        ...
        if "designatingCommitteeName" in sf_schedule_dict:
            sf_schedule_page_dict["designatingCommitteeName"] = sf_schedule_dict[
                "designatingCommitteeName"
            ]

        if "subordinateCommitteeName" in sf_schedule_dict:
            sf_schedule_page_dict["subordinateCommitteeName"] = sf_schedule_dict[
                "subordinateCommitteeName"
            ]
            sf_schedule_page_dict["subordinateCommitteeStreet1"] = sf_schedule_dict[
                "subordinateCommitteeStreet1"
            ]
            sf_schedule_page_dict["subordinateCommitteeStreet2"] = sf_schedule_dict[
                "subordinateCommitteeStreet2"
            ]
            sf_schedule_page_dict["subordinateCommitteeCity"] = sf_schedule_dict[
                "subordinateCommitteeCity"
            ]
            sf_schedule_page_dict["subordinateCommitteeState"] = sf_schedule_dict[
                "subordinateCommitteeState"
            ]
            sf_schedule_page_dict["subordinateCommitteeZipCode"] = sf_schedule_dict[
                "subordinateCommitteeZipCode"
            ]

        if "payeeLastName" in sf_schedule_dict and sf_schedule_dict["payeeLastName"]:
            sf_schedule_page_dict["payeeName_" + str(index)] = (
                sf_schedule_dict["payeeLastName"]
                + ","
            ...
            "payeeOrganizationName" in sf_schedule_dict
            and sf_schedule_dict["payeeOrganizationName"]
        ):
            sf_schedule_page_dict["payeeName_" + str(index)] = sf_schedule_dict[
                "payeeOrganizationName"
            ]
        elif (
            "designatingCommitteeName" in sf_schedule_dict
            and sf_schedule_dict["designatingCommitteeName"]
        ):
            sf_schedule_page_dict["payeeName_" + str(index)] = sf_schedule_dict[
                "designatingCommitteeName"
            ]
        ...
        if "payeeCandidateLastName" in sf_schedule_dict:
            sf_schedule_page_dict["payeeCandidateName_" + str(index)] = (
                sf_schedule_dict["payeeCandidateLastName"]
                + ","
        ...
        if key == "expenditureDate" and sf_schedule_dict["expenditureDate"] not in [
            "none",
            "null",
            " ",
            "",
            ...
            if key == "expenditureAmount" or key == "aggregateGeneralElectionExpended":
                sf_schedule_page_dict[key + "_" + str(index)] = (
                    "{0:.2f}".format(sf_schedule_dict[key])
                    if sf_schedule_dict[key]
                    else 0.0
                )
            else:
        ...
        print(
            "Error at key: "
            + key
            + " in Schedule F transaction: "
            + str(sf_schedule_dict)
        )
        raise e


def build_contributor_la_name_date_dict(
    index, key, la_schedule_dict, la_schedule_page_dict
):

    try:
        # print("la", la_schedule_dict, la_schedule_page_dict, index, key)
        if (
            "contributorLastName" in la_schedule_dict
            and la_schedule_dict["contributorLastName"]
        ):
            la_schedule_page_dict["contributorName_" + str(index)] = (
                la_schedule_dict["contributorLastName"]
                + ","
            ...
            la_schedule_page_dict["contributorName_" + str(index)] = la_schedule_dict[
                "contributorOrgName"
            ]
        ...
        if "contributionAmount" in la_schedule_dict:
            ...
            la_schedule_page_dict[
                "contributionAmount_" + str(index)
            ] = "{0:.2f}".format(la_schedule_dict["contributionAmount"])
        ...
        if "contributionAggregate" in la_schedule_dict:
            ...
            la_schedule_page_dict[
                "contributionAggregate_" + str(index)
            ] = "{0:.2f}".format(la_schedule_dict["contributionAggregate"])
...
def build_slb_name_date_dict(index, key, slb_schedule_dict, slb_schedule_page_dict):
    try:
        ...
        if "payeeLastName" in slb_schedule_dict and slb_schedule_dict["payeeLastName"]:
            slb_schedule_page_dict["payeeName_" + str(index)] = (
                slb_schedule_dict["payeeLastName"]
                + ","
            ...
            slb_schedule_page_dict["payeeName_" + str(index)] = slb_schedule_dict[
                "payeeOrganizationName"
            ]
        ...
        if key == "expenditureDate":
            ...
            if key == "expenditureAmount" or key == "expenditureAggregate":
                slb_schedule_page_dict[key + "_" + str(index)] = "{0:.2f}".format(
                    slb_schedule_dict[key]
                )
            else:
...
def build_sh2_name_date_dict(index, key, sh2_schedule_dict, sh2_schedule_page_dict):
    ...
    try:
        ...
        print(
            "Error at key: "
            + key
            + " in Schedule SH2 tranlaction: "
            + str(sh2_schedule_dict)
        )
        raise e
