...
from routes.src.form3x import calculate_memo_page_count
...
def get_summary_detail(f3l_summary, f3l_data, f3l_data_summary):
    ...
    if (
        f3l_data["semi_annual_start_date"]
        and len(f3l_data["semi_annual_start_date"]) > 0
    ):
    ...
    if (
        f3l_summary["contribute_amount_query"]
        and len(f3l_summary["contribute_amount_query"]) > 0
    ):
        f3l_amount = f3l_summary["contribute_amount_query"]
        f3l_data_summary["quarterly_monthly_total"] = f3l_amount[
            "quarterly_monthly_total"
        ]
...
def print_pdftk(stamp_print):
    ...
    if "json_file" in request.files:
        ...
        f3l_data["treasurerName"] = (
            f3l_data["treasurerLastName"] + "," + f3l_data["treasurerFirstName"]
        )
        ...
        process_output, total_no_of_pages = process_schedules(
            f3l_data, md5_directory, total_no_of_pages
        )
        ...
        if len(f3l_summary) > 0:
            ...
            if "memoText" in f3l_data_summary and f3l_data_summary["memoText"]:
                ...
                memo_dict["scheduleName_1"] = (
                    "F3L" + f3l_data_summary["amendmentIndicator"]
                )
                ...
                pypdftk.concat(
                    [md5_directory + "F3L_Summary.pdf", temp_memo_outfile],
                    md5_directory + json_file_md5 + "_temp.pdf",
                )
                shutil.copy(
                    md5_directory + json_file_md5 + "_temp.pdf",
                    md5_directory + "F3L_Summary.pdf",
                )
                os.remove(md5_directory + json_file_md5 + "_temp.pdf")
            ...
            if has_sa_schedules:
                ...
                shutil.copy(
                    md5_directory + "F3L_Summary.pdf", md5_directory + "all_pages.pdf"
                )
    ...
    if "schedules" in f3l_data:
        ...
        if "SA" in schedules:
            ...
            if sa_schedules:
                ...
                total_no_of_pages = (
                    total_no_of_pages + sa_11a_page_cnt + sa_11a_memo_page_cnt
                )
        ...
        if "SB" in schedules:
            ...
            if sb_schedules_cnt > 0:
                ...
                (
                    sb_17_memo_page_cnt,
                    sb_17_memo_last_page_cnt,
                ) = calculate_memo_page_count(sb_17_memo)

                total_no_of_pages = (
                    total_no_of_pages + sb_17_page_cnt + sb_17_memo_page_cnt
                )
        ...
        if sb_line_page_cnt > 0:
            ...
            for sb_page_no in range(sb_line_page_cnt):
                ...
                if sb_line_page_cnt == (sb_page_no + 1):
                    ...
                    sb_schedule_page_dict["scheduleAggTotal"] = "{0:.2f}".format(
                        schedule_aggregate_total
                    )
...
def build_sb_per_page_schedule_dict(
    ...
    if transactions_in_page == 1:
        for i in range(1):
            page_arg_subtotal, page_subtotal, sb_schedule_dict = building_sb_index(
                i + 1,
                memo_array,
                page_start_index,
                page_arg_subtotal,
...
def building_sb_index(
    ...
    build_payee_name_date_dict(
        index, page_start_index, sb_schedule_dict, sb_schedule_page_dict
    )
...
def build_payee_name_date_dict(index, key, sb_schedule_dict, sb_schedule_page_dict):
    try:
        ...
        if "expenditureAggregate" in sb_schedule_dict:
            ...
            sb_schedule_page_dict[
                "expenditureAggregate_" + str(index)
            ] = "{0:.2f}".format(sb_schedule_dict["expenditureAggregate"])
        ...
        if sa_line_page_cnt > 0:
            for sa_page_no in range(sa_line_page_cnt):
                ...
                if sa_line_page_cnt == (sa_page_no + 1):
                    ...
                    sa_schedule_page_dict["scheduleAggTotal"] = "{0:.2f}".format(
                        page_aggregate_total
                    )
...
def build_sa_per_page_schedule_dict(
    ...
    if transactions_in_page == 1:
        for i in range(1):
            page_arg_subtotal, sa_schedule_dict, page_subtotal = building_sa_index(
                i + 1,
                memo_array,
                page_start_index,
                page_arg_subtotal,
